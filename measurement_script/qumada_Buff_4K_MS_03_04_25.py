...
device.map_terminals(path = rf"C:\Users\lab2\Documents\Userfiles\Sasikala\Scripts\qumada\mapping_Qubus_IndFET_{HERMIT_SLOT}_03.04.json")
...
# device.Independent_FET.add_terminal_parameter(parameter_name="current")
# Independent_FET.current.type = "gettable"
# device.Right_Screening_Gate.add_terminal_parameter(parameter_name="current")
# Right_Screening_Gate.current.type = "gettable"
...
device.Independent_FET.voltage.measured_ramp(-0.5, start=0, name = "Independent FET sweep 0 to -0.5V", 
                                             buffered = False,
                                              num_points = 100, 
                                              backsweep =True)
...
#%%
device.Left_Screening_Gate.voltage.measured_ramp(-0.2, start=0, name = "LEFT Screening Gate", 
                                             buffered = True,
                                              num_points = 200, 
                                              backsweep =True)
...
# device.Right_Screening_Gate.voltage.ramp(0.2)
...
device.Independent_FET.voltage.measured_ramp(0.5, 
                                             name = "Independent FET sweep 0 to 0.5", 
                                             buffered = False,
                                              num_points = 200, 
                                              backsweep =True)
...
Source_Drain_Right.current.break_conditions = ["val > 0.4e-9"]
# Right_Top_Accumulation.current.type = '' 
for gate in right_SET_gates:
    current_voltage = gate()
    gate.voltage.measured_ramp(-0.5, buffered = False,
                               num_points= 100, 
                               backsweep = True)

# %%1D Screening Gate Sweep(Hysterisis)

device.Right_Screening_Gate.voltage.measured_ramp(-0.5, 
                                                  buffered = True,
                                                  num_points = 500,
                                                  name = "SG 0 to -0.3V", 
                                                  backsweep = True)
...
device.sweep_2d(slow_param = Right_Top_Accumulation.voltage, 
                fast_param =Right_Screening_Gate.voltage, 
                slow_param_range = 0.5, 
                fast_param_range = 0.2,
                slow_num_points=50,
                fast_num_points=50, 
                name ="Top-SG sweep", 
                buffered=True)
...
device.sweep_2d(slow_param = Right_Top_Accumulation.voltage, 
                fast_param =Right_Bottom_Accumulation.voltage, 
                slow_param_range = 0.5, 
                fast_param_range = 0.5,
                slow_num_points=50,
                fast_num_points=50, 
                name ="BB sweep", 
                buffered=True)
